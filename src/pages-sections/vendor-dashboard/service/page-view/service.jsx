...
const ServicePageView = () => {
  ...
  try {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(expiryDateStr)) {
      ...
      expiryDate = new Date(parseInt(year), monthMap[month], parseInt(day));
    }
      ...
      const interval = setInterval(async () => {
        try {
          const response = await fetch(
            `/api/service/verification/${sessionId}`
          );
          if (response.ok) {
        ...
        {!sessionId ? (
          ...
          <Grid item xs={12} md={6} mx="auto">
            <Card elevation={3} sx={{ borderRadius: 3 }}>
              <CardContent sx={{ textAlign: "center", p: 4 }}>
                ...
                <Button variant="outlined" onClick={resetService} sx={{ mt: 2 }}>
                  Cancel Session
                </Button>
              </CardContent>
            </Card>
          ...
          <Grid item xs={12}>
            <Card>
              ...
              {/* 🔴 Option 2: Global warning banner if expired */}
              {isDocumentExpired(passportData.expiryDate) && (
                <Alert severity="error" icon={<WarningIcon />} sx={{ m: 0 }}>
                    ...
                    <Grid container spacing={2}>
                      ...
                      <Grid item xs={12} sm={6}>
                        ...
                        <Box sx={{ mb: 2 }}>
                          ...
                          <Typography variant="h6">
                            {passportData.issuingCountry || "N/A"}
                          </Typography>
                        </Box>
