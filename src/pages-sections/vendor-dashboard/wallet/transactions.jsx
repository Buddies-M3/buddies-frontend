...
const WalletTransactions = ({ walletAddress }) => {
  ...
  const fetchTransactionHistory = async (filters) => {
    if (walletAddress) {
      try {
        let filter = `owner: "${walletAddress.toLowerCase()}"`;
        ...
        const query = `
          {
            tokens(first: ${pageSize}, skip: ${(pageKey) * pageSize}, where: ${filterString}) {
              amount
              ipfsHashURI {
                projectType
                projectName
  ...
  const handleAction = async () => {
    try {
      ...
      const erc1155 = new ethers.Contract(contractAddress, GreenChainsCoinABI, signer);

      try {
        // Execute the burn function
        const tx = await erc1155.burnBatch(selectedRows);
        await tx.wait();
        console.log(`Token ID ${tokenId} burned successfully`);
      } catch (burnError) {
        console.error(`Error burning token ID ${tokenId}:`, burnError);
      }

      // Iterate over selectedRows to burn tokens
      /* for (let i = 0; i < selectedRows.length; i++) {
        const tokenId = selectedRows[i];

        try {
          // Execute the burn function
          const tx = await erc1155.burn(tokenId);
          await tx.wait();
          console.log(`Token ID ${tokenId} burned successfully`);
        } catch (burnError) {
          console.error(`Error burning token ID ${tokenId}:`, burnError);
    ...
    <Box py={4}>
      ...
      <Card>
        <Box p={2} display="flex" justifyContent="flex-start">
          <StyledButton variant="contained" color="secondary" onClick={handleAction} disabled={burning || selectedRows <= 0}>
            {burning ? <CircularProgress size={24} /> : "Retire"}
          </StyledButton>
        </Box>
        <Divider />
